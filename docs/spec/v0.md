...
- In Frozen mode, a dragged region selection can enter scroll capture via `s` or the
  toolbar `Scroll` action; manual scrolling appends stable samples into a side preview,
...
9. Frozen-mode scroll capture:
   ...
   - Entry points are plain `s` in Frozen mode or the frozen toolbar `Scroll` action.
   - While active, the overlay becomes pass-through so the underlying app can be manually
     scrolled, and rsnap appends stable samples from the selected region into a side preview.
