...
- In Frozen mode, a dragged region selection can enter scroll capture via `s` or the
  toolbar `Scroll Capture ↓` action; manual downward scrolling appends stable samples into a side preview,
  `Space` / Cmd+S / Ctrl+S exports the stitched image, and `Esc` / `Back` returns to
  the original Frozen capture.
...
9. Frozen-mode scroll capture:
   - Scroll capture is available only after a dragged region freeze; fullscreen fallback and
     window/fallback freezes do not enter scroll capture.
   - Entry points are plain `s` in Frozen mode or the frozen toolbar `Scroll Capture ↓`
     action.
   - While active, the overlay becomes pass-through so the underlying app can be manually
     scrolled downward, and rsnap appends stable samples from the selected region into a
     side preview.
   - `Space` copies the stitched image and exits, Cmd+S (macOS) / Ctrl+S saves it and exits,
     and `Esc` / `Back` stops scroll capture and restores the original Frozen capture.
