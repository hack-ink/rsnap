//! Native-host wrapper around the scroll-capture stitching engine.
...
impl ScrollStitchSession {
	...
	/// Observes a discrete native screenshot using worker-pairwise registration.
	pub fn observe_worker_pairwise_rgba(
		...
		self.inner.observe_worker_pairwise_frame(frame).map(scroll_stitch_observe_outcome_from)
	}
	...
	pub fn observe_downward_rgba_with_motion_hint(
		...
		self.inner
			.observe_worker_pairwise_frame_with_motion_hint(frame, motion_rows_hint)
...
fn scroll_stitch_observe_outcome_from(value: ScrollObserveOutcome) -> ScrollStitchObserveOutcome {
	match value {
