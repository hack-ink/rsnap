...
fn decode_png_streaming_area_thumbnail(
	...
	let Some((destination_width, destination_height)) =
		...
		return Err(eyre!("PNG wallpaper has invalid dimensions"));
	};
	...
	if source_y != source_height as usize {
		return Err(eyre!("decoded {source_y} PNG rows, expected {source_height}"));
	}

	let mut rgba = vec![0u8; accumulator.len()];
...
fn resize_rgba_lanczos_to_fit(
	...
	let Some((destination_width, destination_height)) =
		...
		return Err(eyre!("PNG wallpaper thumbnail has invalid dimensions"));
	};
...
fn expected_rgba_len(width: u32, height: u32) -> Result<usize> {
	(width as usize)
		...
		.ok_or_else(|| eyre!("RGBA image dimensions overflow"))
}
...
impl PngRowLayout {
	fn from_output((color_type, bit_depth): (ColorType, BitDepth)) -> Result<Self> {
		if bit_depth != BitDepth::Eight {
			return Err(eyre!("unsupported PNG bit depth after transformations: {bit_depth:?}"));
		}

		match color_type {
			ColorType::Rgba => Ok(Self::Rgba),
			ColorType::GrayscaleAlpha => Ok(Self::GrayscaleAlpha),
			other => Err(eyre!("unsupported PNG color type after transformations: {other:?}")),
		}
	}
	...
	fn expected_len(self, width: u32) -> Result<usize> {
		(width as usize)
			...
			.ok_or_else(|| eyre!("PNG row length overflow"))
	}
...
mod tests {
	...
	fn png_thumbnail_downsamples_without_full_frame_buffer() {
		...
		let thumbnail = capture_frame_wallpaper_png_thumbnail(&path, 2)
	...
	fn png_thumbnail_skips_non_png_paths() {
		let thumbnail = capture_frame_wallpaper_png_thumbnail("/tmp/not-a-wallpaper.jpg", 128)
	...
	fn png_thumbnail_decodes_rgb_and_grayscale_inputs() {
		let rgb_path = std::env::temp_dir().join(format!(
			"rsnap-wallpaper-thumb-{}-{}.png",
			std::process::id(),
			"rgb"
		));
		let grayscale_path = std::env::temp_dir().join(format!(
			"rsnap-wallpaper-thumb-{}-{}.png",
			std::process::id(),
			"grayscale"
		));
		...
		let rgb_thumbnail = capture_frame_wallpaper_png_thumbnail(&rgb_path, 3)
			.expect("RGB test PNG should decode")
			.expect("RGB PNG thumbnail should be produced");
		let grayscale_thumbnail = capture_frame_wallpaper_png_thumbnail(&grayscale_path, 3)
