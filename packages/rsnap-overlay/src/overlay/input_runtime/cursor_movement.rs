...
use crate::overlay::{
	CursorMoveTrace, DeviceCursorPointSource, FrozenSelectionDragCursorMoveTiming, GlobalPoint,
	MonitorRect, OverlayControl, OverlayMode, OverlaySession, PhysicalPosition, PhysicalSize,
	WindowId,
};

impl OverlaySession {
	#[cfg(not(target_os = "macos"))]
	pub(in crate::overlay) fn sample_mouse_location(&mut self) -> GlobalPoint {
	...
	pub(in crate::overlay) fn sample_mouse_location(&mut self) -> GlobalPoint {
		...
		let point = macos_window_bridge::macos_mouse_location().unwrap_or(GlobalPoint::new(0, 0));
	...
	pub(in crate::overlay) fn last_fresh_event_cursor(&self) -> Option<(MonitorRect, GlobalPoint)> {
		self.last_fresh_event_cursor_with_ttl(CURSOR_EVENT_TICK_TTL)
	}

	pub(in crate::overlay) fn last_fresh_event_cursor_with_ttl(
		&self,
		ttl: Duration,
	...
	pub(in crate::overlay) fn handle_cursor_moved(
		&mut self,
		window_id: WindowId,
	...
	#[cfg(target_os = "macos")]
	pub(in crate::overlay) fn handle_native_overlay_pointer_moved(
		&mut self,
		monitor: MonitorRect,
		global: GlobalPoint,
	) -> OverlayControl {
	...
	pub(in crate::overlay) fn handle_cursor_moved_with_overlay_window(
		&mut self,
		window_id: WindowId,
		position: PhysicalPosition<f64>,
	...
	pub(in crate::overlay) fn overlay_window_event_global_position(
		window_monitor: MonitorRect,
		scale_factor: f64,
		window_size: PhysicalSize<u32>,
		position: PhysicalPosition<f64>,
	...
	pub(in crate::overlay) fn overlay_window_frozen_selection_drag_global_position(
		window_monitor: MonitorRect,
		scale_factor: f64,
		window_size: PhysicalSize<u32>,
		position: PhysicalPosition<f64>,
	...
	pub(in crate::overlay) fn current_device_cursor(&mut self) -> GlobalPoint {
		self.sample_mouse_location()
	}

	pub(in crate::overlay) fn current_frozen_interaction_cursor(&mut self) -> GlobalPoint {
	...
	pub(in crate::overlay) fn should_trace_frozen_selection_drag_timing(&self) -> bool {
		tracing::enabled!(tracing::Level::TRACE)
			&& matches!(self.state.mode, OverlayMode::Frozen)
			&& self.frozen_selection_drag.active
	}

	pub(in crate::overlay) fn update_cursor_for_live_move(
		&mut self,
		old_monitor: Option<MonitorRect>,
		old_cursor: Option<GlobalPoint>,
		monitor: MonitorRect,
