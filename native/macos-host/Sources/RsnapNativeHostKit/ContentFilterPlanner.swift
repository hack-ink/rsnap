...
struct FrozenDisplayTarget: Equatable, Sendable {
...
enum ContentFilterPlanner {
	static func displayTarget(for screen: NSScreen) -> FrozenDisplayTarget? {
		guard let displayID = screen.nativeDisplayID else {
			return nil
		}
		let scale = max(screen.backingScaleFactor, 1)
		return FrozenDisplayTarget(
			displayID: displayID,
			frame: screen.frame,
	...
	static func streamConfiguration(for target: FrozenDisplayTarget) -> SCStreamConfiguration {
	...
	static func filtersAreComplete(
		...
		for targets: [FrozenDisplayTarget]
	) -> Bool {
	...
	private static func contentFilter(
		for target: FrozenDisplayTarget,
		in content: SCShareableContent,
		selfCaptureExceptionWindowIDs: Set<CGWindowID>,
		includedCurrentProcessWindowIDs: Set<CGWindowID>
