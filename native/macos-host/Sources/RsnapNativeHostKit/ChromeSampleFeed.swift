...
final class ChromeSampleFeed: @unchecked Sendable {
	...
	func snapshot(for point: CGPoint?) -> LiveChromeSample? {
		...
		if let latestSamplePoint,
			ChromeSamplePolicy.pointsEquivalent(latestSamplePoint, point)
		{
			return latestSample
		}
	...
	private func refresh() {
		...
		let framePatchSample =
			...
			: nil
		let reusableRgbSample = ChromeSamplePolicy.reusableRgbSample(
			previousSample: previousSample, previousPoint: previousPoint, point: point, now: now)
		let rgbSample =
			frameRgbSample
			?? reusableRgbSample
		let rgbSource =
			ChromeSamplePolicy.rgbSampleSource(
		...
		let patchSample =
			ChromeSamplePolicy.sampleWithUpdatedPatch(
				rgb: nil, loupePatch: framePatchSample)
			?? ChromeSamplePolicy.recentPatchSample(
				previousSample: previousSample,
				canReuseRecentPatch: canReuseRecentPatch
			)
			?? ChromeSamplePolicy.reusablePatchSample(
				previousSample: previousSample,
				previousPoint: previousPoint,
				point: point,
				includeLoupePatch: includeLoupePatch
			)
		let sample = ChromeSamplePolicy.sampleWithUpdatedPatch(
			rgb: rgbSample,
	...
	private func enqueueBackgroundSampleIfNeeded(
		...
		stateLock.lock()
		if let streamRgbSample, !ChromeSamplePolicy.isLikelyOverlayWhite(streamRgbSample) {
			backgroundCorrectionMode = false
			whiteStreamRunHasProbed = false
			stateLock.unlock()
			return
	...
	private func refreshBackgroundSample(
		...
		guard let desiredPoint,
			sample != nil,
			desiredSource == source,
			ChromeSamplePolicy.pointsEquivalent(desiredPoint, point)
		else {
		...
		if shouldProbeForCorrection, let rgbSample = sampleRgbValue {
			backgroundCorrectionMode = !ChromeSamplePolicy.isLikelyOverlayWhite(
				rgbSample)
		}
		...
		let backgroundSampleTelemetry =
			...
			? BackgroundSampleTelemetry(
				captureID: activeCaptureID,
				totalMilliseconds: sampleMilliseconds,
				outcome: ChromeSamplePolicy.backgroundSampleOutcome(
		...
		let shouldNotify =
			...
			|| ChromeSamplePolicy.shouldNotifySampleUpdated(
