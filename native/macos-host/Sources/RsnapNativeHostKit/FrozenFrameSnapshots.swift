...
extension FrozenFrameAuthority {
	...
	func liveRgbSample(containing point: CGPoint) -> LiveRgbSample? {
		...
		guard
			let rgb = PixelBufferBridge.rgbSample(
	...
	func loupePatch(containing point: CGPoint, sidePixels: Int) -> CGImage? {
		...
		return PixelBufferBridge.loupePatch(
			from: record.pixelBuffer,
			point: point,
			displayFrame: record.displayFrame,
	...
	func regionImage(in rect: CGRect) -> CGImage? {
		...
		return PixelBufferBridge.regionImage(
			from: record.pixelBuffer,
			rect: rect,
			displayFrame: record.displayFrame
		)
	...
	func nextRegionFrame(
		...
		guard
			...
			let region = PixelBufferBridge.regionSnapshot(
				from: record.pixelBuffer,
				rect: rect,
				displayFrame: record.displayFrame
			)
	...
	func snapshot(
		...
		guard let record,
			let image = PixelBufferBridge.makeImage(from: record.pixelBuffer)
		else {
			return nil
		}
