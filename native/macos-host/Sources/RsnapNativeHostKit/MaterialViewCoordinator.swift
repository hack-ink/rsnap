...
@MainActor
final class MaterialViewCoordinator {
	...
	private var scrollToolbarBackdropState = ToolbarBackdropState()
	private var glassPatchResolver = GlassPatchResolver()
	...
	private var toolbarHoverState: ToolbarHoverState { hostView.toolbarHoverState }
	...
	func glassPatch(
		for surfaceKind: GlassSurfaceKind,
		frame: CGRect
	) -> CGImage? {
	...
	private func frozenDisplayPatch(
		...
		GlassPatchResolver.frozenDisplayPatch(
			in: globalFrame,
	...
	private func scheduleScrollToolbarBackdropCapture(
		...
		DispatchQueue.main.async { [weak self] in
			...
			self.scrollToolbarBackdropCaptureQueue.async {
				...
				let result = ToolbarBackdropWorker.captureResult(
					in: globalFrame,
	...
	private func finishScrollToolbarBackdropCapture(
		...
		recordToolbarBackdropChangeIfNeeded(signature: signature)
	}

	private func recordToolbarBackdropChangeIfNeeded(signature: UInt64?) {
		let now = ProcessInfo.processInfo.systemUptime
