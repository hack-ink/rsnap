...
package enum PixelBufferRegionProbe {
	package static func verifyRegionSnapshotMapping() {
		...
		guard
			let scaledRegion = PixelBufferBridge.regionSnapshot(
				from: pixelBuffer,
		...
		guard
			let clippedRegion = PixelBufferBridge.regionSnapshot(
				from: pixelBuffer,
		...
		if PixelBufferBridge.regionSnapshot(
			from: pixelBuffer,
