...
struct SharedScrollInputSnapshot {
	seq: u64,
	delta_y: f64,
	global_x: f64,
	global_y: f64,
...
struct SharedScrollInputState {
	enabled: AtomicBool,
	snapshot: Mutex<SharedScrollInputSnapshot>,
	next_seq: AtomicU64,
}
#[cfg(target_os = "macos")]
impl SharedScrollInputState {
	...
	fn clear(&self) {
		...
		*snapshot = SharedScrollInputSnapshot::default();
	}
	...
	fn record(
		...
		let (effective_delta_y, effective_global_x, effective_global_y) =
			if gesture_ended && delta_y == 0.0 && snapshot.delta_y != 0.0 {
				(snapshot.delta_y, snapshot.global_x, snapshot.global_y)
			} else {
				(delta_y, global_x, global_y)
			};

		*snapshot = SharedScrollInputSnapshot {
			seq,
	...
	fn snapshot_tuple(&self) -> Option<(u64, f64, f64, f64, bool, bool)> {
		...
		Some((
			...
			snapshot.global_y,
			snapshot.delta_y,
			snapshot.gesture_active,
			snapshot.gesture_ended,
		))
	}
}
...
impl App {
	...
	fn start_capture_session(&mut self, event_loop: &ActiveEventLoop, requested_by: &'static str) {
		...
		#[cfg(target_os = "macos")]
		overlay_session.set_external_scroll_input_snapshot_reader(Arc::new({
			let shared_state = Arc::clone(&self.scroll_input_shared_state);

			move || shared_state.snapshot_tuple()
		}));
...
mod tests {
	...
	use crate::app::{
		...
		NSEVENT_PHASE_ENDED, NSEVENT_PHASE_MAY_BEGIN, SharedScrollInputState,
	...
	fn terminal_scroll_event_preserves_last_effective_delta() {
		...
		state.record(0.0, 0.0, 0.0, false, true);

		assert_eq!(state.snapshot_tuple(), Some((2, 120.0, 140.0, -4.0, false, true)));
	}
	...
	fn newer_non_zero_scroll_event_replaces_preserved_delta() {
		...
		state.record(-4.0, 120.0, 140.0, true, false);
		state.record(0.0, 0.0, 0.0, false, true);
		state.record(6.0, 220.0, 260.0, true, false);

		assert_eq!(state.snapshot_tuple(), Some((3, 220.0, 260.0, 6.0, true, false)));
	}
}
