...
description: "Executable procedure and verification guidance for Scroll-Capture Recovery Plan."
type: "Runbook"
status: active
authority: normative
owner: acg-box/rsnap
...
Inputs: [`openwiki/spec/capture-session.md`](../spec/capture-session.md); [`openwiki/decisions/scroll-capture-architecture.md`](../decisions/scroll-capture-architecture.md);
[`openwiki/reference/smoke-perf-validation-surface.md`](../reference/smoke-perf-validation-surface.md); current user-visible live failures.
...
Authoritative prior-art analysis lives in [`openwiki/decisions/scroll-capture-architecture.md`](../decisions/scroll-capture-architecture.md) and the
research contract at [`openwiki/research/scroll-capture-prior-art-2026-05-10.md`](../research/scroll-capture-prior-art-2026-05-10.md).
...
The [Scroll Capture Architecture](../decisions/scroll-capture-architecture.md) supplies the accepted rationale, while the [Capture Session Contract](../spec/capture-session.md) owns user-visible correctness.

```mermaid
flowchart TD
    A["Receive overlay-local wheel input"] --> B["Forward during passthrough window"]
    B --> C["Acquire ordered selected-region frame"]
    C --> D{"Registration and overlap proven"}
    D -->|Yes| E["Commit monotonic downward growth"]
    D -->|No| F["Leave committed canvas unchanged"]
    F --> G{"Committed frontier reacquired"}
    G -->|Yes| C
    G -->|No| F
    E --> H["Render one canvas for preview copy and save"]
```

The flow shows the fail-closed append and rewind-reacquisition rules enforced by the native sampling path and Rust stitcher.
