...
description: "Executable procedure and verification guidance for Validate Host/Core Reset Work."
type: "Runbook"
status: active
authority: normative
owner: acg-box/rsnap
...
[`openwiki/spec/capture-session.md`](../spec/capture-session.md), [`openwiki/spec/platform-host-boundary.md`](../spec/platform-host-boundary.md), and
[`openwiki/spec/performance.md`](../spec/performance.md).

Depends on: [`openwiki/spec/capture-session.md`](../spec/capture-session.md); [`openwiki/spec/platform-host-boundary.md`](../spec/platform-host-boundary.md);
[`openwiki/runbook/performance-validation.md`](./performance-validation.md)
...
2. Re-read [`openwiki/quickstart.md`](../quickstart.md), [`openwiki/policy.md`](../policy.md), and every touched lane index to confirm they route to
   the same active document set.
3. Verify every `openwiki/*.md` path reference in the documentation resolves:
...
root = Path(".").resolve()
wiki = root / "openwiki"
pattern = re.compile(r"`(openwiki/[A-Za-z0-9_./-]+\.md)`")
missing = []
...
if missing:
    ...
    raise SystemExit(1)
print("All openwiki/*.md path references resolve.")
PYCODE
```

4. If the change rewires active routing, run
   `rg -n "historical|superseded" openwiki/quickstart.md openwiki/policy.md openwiki/spec openwiki/reference openwiki/runbook openwiki/decisions`
