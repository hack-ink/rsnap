...
description: "Current-state implementation and ownership reference for Live Sampling Reference."
type: "Reference"
status: active
authority: normative
owner: acg-box/rsnap
...
Inputs: [`openwiki/spec/capture-session.md`](../spec/capture-session.md); current focus is the macOS live path rather than
freeze/export.

Depends on: [`openwiki/spec/capture-session.md`](../spec/capture-session.md)
...
Spec boundary: [`openwiki/spec/capture-session.md`](../spec/capture-session.md)

Governing performance contract: [`openwiki/spec/performance.md`](../spec/performance.md)
...
architecture. For the reset architecture route, start with [`openwiki/reference/host-core-reset.md`](./host-core-reset.md) and
...
This current-state implementation supports the [Capture Session Contract](../spec/capture-session.md) and is constrained by the [Performance Contract](../spec/performance.md).

```mermaid
sequenceDiagram
    participant Cursor as Cursor Event
    participant Windows as Window Snapshot Cache
    participant Frames as Frozen Frame Authority
    participant UI as HUD and Loupe
    Cursor->>Windows: Resolve hovered window locally
    Cursor->>Frames: Sample latest eligible stream frame
    Frames-->>Cursor: RGB and loupe pixels
    Windows-->>Cursor: Highlighted window
    Cursor->>UI: Present sampled scene
```

The sequence shows why cursor movement avoids synchronous full-display capture and per-event window-server queries.
